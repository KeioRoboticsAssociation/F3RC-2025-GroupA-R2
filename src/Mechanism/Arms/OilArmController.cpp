#include "Mechanism/Arms/OilArmController.hpp"
...
void OilArmController::update() {
    switch (state_) {
        ...
        case State::kInitialize:
            arm_servo_.setAngleDeg(oil_arm_config::kArmInitialAngle);
            gripper_servo_.setAngleDeg(oil_arm_config::kGripperOpenAngle);
            if (!arm_servo_.isHomed() && !gripper_servo_.isHomed()) {
        ...
        case State::kCollectStart:
            ...
            arm_servo_.setAngleDeg(oil_arm_config::kArmExtendAngle);
            gripper_servo_.setAngleDeg(oil_arm_config::kGripperOpenAngle);
            if (!arm_servo_.isHomed() && !gripper_servo_.isHomed()) {
        ...
        case State::kCollectGrip:
            // Grip the oil
            gripper_servo_.setAngleDeg(oil_arm_config::kGripperCloseAngle);
            if (!gripper_servo_.isHomed()) {
        ...
        case State::kCollectLift:
            // Lift the oil
            arm_servo_.setAngleDeg(oil_arm_config::kArmLiftAngle);
            if (!arm_servo_.isHomed()) {
        ...
        case State::kPlaceStart:
            ...
            arm_servo_.setAngleDeg(oil_arm_config::kArmPlaceAngle);
            if (!arm_servo_.isHomed()) {
...
void OilArmController::stop() {
    ...
    arm_servo_.setAngleDeg(arm_servo_.getAngleDeg());
    gripper_servo_.setAngleDeg(gripper_servo_.getAngleDeg());
    setState(State::kIdle);
}
