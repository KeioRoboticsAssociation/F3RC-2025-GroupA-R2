...
/**
 * @brief Section5: 壁衝突スタート → 前進 → ToF検知で減速 → 発電所の壁に衝突 → -Y軸方向に直進 → 壁に衝突 → 座標リセット
 * 
 * このセクションでは以下の制御を行います：
 * 1. 壁衝突後の状態から開始
 * 2. 前進開始
 ...
 * 4. 発電所の壁に衝突（100mm以下）
 * 5. そのまま画像下方向（-Y軸方向）に直進
 * 6. 壁に衝突（100mm以下）
 * 7. 壁衝突時にStateEstimatorをリセット
 */
class Section5 : public BaseSection {
public:
    /**
     * @brief コンストラクタ
     ...
     */
    Section5(StateEstimator& state, BehaviorController& behavior);
...
private:
    ...
    bool power_plant_collision_;
    bool moving_to_negative_y_;
